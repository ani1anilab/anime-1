  ...
  try {
    ...
    const promises = slideElements
      .map(async (ind, ele) => {
        ...
        const hrefgetseason = $(ele).find('.deslide-item > .deslide-item-content > .desi-buttons > a:eq(0)').attr('href').replace('/', '')
        const tvInfoMapping = {
          0: "showType",
          1: "duration",
          2: "releaseDate",
        ...
        return { hrefgetseason, data_id, poster, title, description, tvInfo };
      })
      .get();
